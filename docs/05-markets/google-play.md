...
## 스토어 페이지 404의 원인 (2026-09-22 콘솔 조사)

승계 업로드 후 `play.google.com/store/apps/details?id=com.etlegame.reversi`가 국가·언어를 바꿔도 **HTTP 404**였다. 원인을 Play Console에서 확인했다.

- 앱 이용 가능 여부는 **`출시됨`**이다(`테스트 및 출시 > 고급 설정`). 게시 취소 상태가 아니다. `게시 취소` 버튼은 비활성이다.
- 정책 상태 화면에 다음이 떠 있다.

  > 대상 API 수준보다 높은 Android 버전의 신규 사용자에게 앱이 제공되지 않습니다.
  > 앱이 Android 15(API 수준 35) 이상을 타겟팅해야 함
  > **앱 사용 가능 여부 감소 날짜: 2023년 8월 31일**

- 프로덕션에서 **실제 제공 중인 버전은 여전히 `2.1.2`**(2015년 옛 빌드)다. 이 빌드가 Target API 요구를 못 맞춰 **2023년 8월부터 신규 사용자에게 숨겨졌고**, 그래서 스토어 페이지가 404다.
- `v2.2.8`(versionCode 50)은 **`검토 중`**이다. 심사를 통과해 제공이 시작되면 이 제한이 풀린다.
- `targetSdk`는 충족한다. `godot/android/build/config.gradle`의 `targetSdk : 36`이고, 실제 산출물도 `targetSdkVersion='36'`로 확인했다. 정책 센터의 "API 36 미만 업데이트 거부"(시행일 2026-08-31)에 걸리지 않는다.
- 앱 콘텐츠에 `주의 필요`로 남은 항목은 없다.

**조치는 필요 없다. 심사 통과를 기다리면 된다.**

### 함정: Play API의 `status=completed`는 심사 통과가 아니다

`edits.tracks().get(track="production")`이 돌려주는 릴리스 `status=completed`는 **롤아웃 비율이 100%라는 뜻일 뿐**이다. 같은 시점에 Play Console은 같은 릴리스를 `검토 중`으로 표시한다. API만 보고 "심사를 통과해 공개됐다"고 읽으면 안 된다.

### 미해결 관측

- 계정 홈의 앱 목록 행은 앱 상태를 `출시 안됨`, 업데이트 상태를 `아직 검토를 위해 전송되지 않음`으로 표시한다. 앱 내부 설정(`출시됨`)과 모순된다. 목록 캐시 지연으로 보이지만 화면 근거로 확정하지 못했다.
- `edits.details`의 `contactWebsite`가 아직 2015년 원 개발사 값(`https://www.facebook.com/reversionline`)이다. 승계 때 정리되지 않은 결함이며, 스토어에 공개되는 연락처다. `https://www.seorilabs.com/support/`로 교체해야 하지만 외부 공개 값이라 사용자 승인 후 바꾼다.
