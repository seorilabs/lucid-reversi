...
- save DTO round trip
- 고정 seed별 BEGINNER 무작위 분기와 EASY/MEDIUM/HARD 동점 최선 수 분산, 동일 seed save/restore 재현, EASY 이상 난이도의 최선 평가 유지
- BEGINNER 대 EASY 색상 교대 self-play의 완주, BEGINNER보다 높은 EASY 승수·코너 점유, 알 수 없는 기존 난이도의 MEDIUM 폴백
...
- BEGINNER/EASY/MEDIUM/HARD 사고 지연 기본값 순서, ±0.04초 흔들림 범위·실제 샘플 변동, 탐색 경과 시간 차감, 대기 중 턴 변경 시 계산된 수 폐기
...
- 모바일 터치 smoke는 주요 플레이 버튼이 56px 이상 높이로 노출되고, 설정 메뉴 안의 4개 난이도 버튼이 한 행에 넘침 없이 배치되며 보드/돌/언어 선택과 함께 작은 select box가 아닌 큰 세그먼트 버튼으로 보이는지 확인한다.
...
- AI 사고 지연 smoke는 4개 난이도별 중앙 상수와 흔들림 경계, 반복 샘플 차이, 탐색 시간이 목표 시간을 넘으면 추가 대기 0, 대기 중 턴 변경 뒤 보드·수순 불변과 pending 해제를 확인한다.
