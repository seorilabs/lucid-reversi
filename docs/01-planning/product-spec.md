...
- In-play fun feedback should stay close to the board: a single theme-derived board surface with thin grid lines, last-move highlight, default-on legal move markers, midpoint color-swap disc flip with a placement-origin wave, placement sound, flip sound, big-flip sound, a low-volume ambient BGM loop, and a full-width advantage meter. BGM and SFX use separate audio buses and independent settings. Classic uses the traditional green Othello surface while Arctic and Ember derive their own surface/grid pair.
